...
export function RequestForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      if (email) await subscribeEmail(email, 'template-request')
    } catch {}
    const body = `Template: ${name}\n\nDescription: ${description}\n\nEmail: ${email}`
    window.location.href =
      `mailto:info@ums-solutions.com?subject=${encodeURIComponent(`Template Request: ${name}`)}&body=${encodeURIComponent(body)}`
    setDone(true)
    setSubmitting(false)
  }
  ...
  return (
    <div className="absolute bottom-6 right-6 z-40 flex flex-col items-end gap-3">
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            {done ? (
              <p className="text-xs text-ums-muted text-center py-4">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full py-2.5 rounded-md text-xs font-bold uppercase tracking-[0.15em] transition-opacity hover:opacity-90 disabled:opacity-60"
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-2 px-4 py-2.5 rounded-full text-xs font-bold uppercase tracking-[0.15em] transition-opacity hover:opacity-90 shadow-lg"
        style={{ background: '#AB9C7D', color: '#1A1918' }}
      >
